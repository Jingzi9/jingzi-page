...
const serve=()=>{
  ...
  bs.init({
    ...
    server:{
      ...
      baseDir:[config.build.temp,config.build.dist,config.build.public],//当请求过来后依次在数组中的文件寻找。如一个页面请求或者样式，脚本请求回到编译后的dist目录寻找，图片请求，在开发阶段在dist目录或者src都无差别，public同理，可减少任务构建
